...
<?php
  ...
  if (isset($_POST['projectType'])) {
	...
	if ($isFormCompleted)
	{
        $sql = "INSERT INTO project (projectName, projectStart, projectFinish, budget, workingDate, satisfaction, statusProject, paid, responId, strategyId, projectType) 
                    VALUES 
                    (
                      '$projectName', 
                      STR_TO_DATE('$projectStart', '%Y-%m-%d'), 
                      STR_TO_DATE('$projectFinish', '%Y-%m-%d'), 
                      $budget, 
                      NULL,
                      NULL,
                      NULL,
